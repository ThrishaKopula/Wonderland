...
func _on_Football_body_entered(body):
	if (body.name == 'player' and StoryVariables.isGetIngredientsFromVendors == true and StoryVariables.isPleb_CItemCollected == false):
		active = true

func _on_Football_body_exited(body):
