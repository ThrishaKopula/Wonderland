	...
	if get_node_or_null('DialogNode') == null:
		if event.is_action_pressed("interact") and active:
			if(StoryVariables.isInitiateRoyalGiftsQuest == true):
				pause_game()
				var dialog = Dialogic.start("butterKnife")
				dialog.pause_mode = Node.PAUSE_MODE_PROCESS
				dialog.connect('timeline_end', self, 'unpause')
				add_child(dialog)
				StoryVariables.diamondItem = true
...
func _on_Knife_body_entered(body):
	if (body.name == 'player' and StoryVariables.isInitiateRoyalGiftsQuest == true and StoryVariables.diamondItem == false):
		active = true
