	...
	if get_node_or_null('DialogNode') == null:
		if event.is_action_pressed("interact") and active:
			if(StoryVariables.isInitiateRoyalGiftsQuest == true):
				pause_game()
				var dialog = Dialogic.start("luckyCharm")
				dialog.pause_mode = Node.PAUSE_MODE_PROCESS
				dialog.connect('timeline_end', self, 'unpause')
				add_child(dialog)
				StoryVariables.cloverItem = true
...
func _on_Clover_body_entered(body):
	if (body.name == 'player' and StoryVariables.isInitiateRoyalGiftsQuest == true and StoryVariables.cloverItem == false):
		active = true

func _on_Clover_body_exited(body):
